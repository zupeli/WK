...
type
  ...
  public
    ...
    procedure TotalizadorCliente(pIdCliente: integer; var rQtd: integer;
              var rValTotal, rValUnit: Extended);
  end;
...
const
   ...
   QryPedidosCliente = 'SELECT c.id_cliente, c.ds_nome, p.ds_produto,        ' +
                       '       pe.id_pedido, pe.dt_pedido, pi.qt_item,       ' +
                       '       format(pi.vl_unitario, 2) AS vl_unitario, ' +
                       '       format(pi.vl_total, 2) AS vl_total ' +
                       '  FROM clientes c                                    ' +
                       ...
                       ' ORDER BY pe.dt_pedido, p.ds_produto, pi.qt_item';
var
  DM: TDM;
...
begin
  ...
  FDConnection.Params.Values['OptimizedNumerics'] := 'False';
...
begin
  ...
  try
    ...
    begin
      ...
      vQry.ParamByName('id_produto' ).AsInteger := vCds.FieldByName('id_produto' ).AsInteger;
...
begin
  ...
  try
    ...
    vQry.SQL.Text := QryPedidosCliente;
    vQry.ParamByName('id_cliente').AsInteger := pIdCliente;
    vDSP.DataSet := vQry;
    vCDS.SetProvider(vDSP);
    vCDS.Open;
...
procedure TDM.TotalizadorCliente(pIdCliente:integer; var rQtd : integer; var rValTotal, rValUnit : Extended);
var
 vQry : TFDQuery;
begin
  vQry := TFDQuery.Create(nil);
  try
    ...
    vQry.SQL.Text := 'SELECT sum(pi.qt_item) as qt_item,         ' +
                     '       sum(pi.vl_unitario) as vl_unitario, ' +
                     '       sum(pi.vl_total) as vl_total        ' +
                     '  FROM pedidos pe                          ' +
                     '  JOIN pedido_item pi ON pe.id_pedido = pi.id_pedido ' +
                     ' WHERE pe.id_cliente = :id_cliente';
    vQry.ParamByName('id_cliente').AsInteger := pIdCliente;
    vQry.Open;
    rQtd      := vQry.FieldByName('qt_item'    ).AsInteger;
    rValUnit  := vQry.FieldByName('vl_unitario').AsFloat;
    rValTotal := vQry.FieldByName('vl_total'   ).AsFloat;
